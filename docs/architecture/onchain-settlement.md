...
Settlement swap paths use DeepBook pools. Path data comes from the `NETWORK` section in `packages/app-api/settlement-swap-paths.json`, and the Host derives path details from on-chain pool data at boot.
