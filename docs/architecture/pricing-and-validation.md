...
| Prefix value and funding resolution             | Generic prepare build and read-only settlement-funding check | Traces direct coin identity and command-ordered split/merge value, then accounts for same-token `FundsWithdrawal(Sender)` use before evaluating the exact funding source or returning bounded incomplete evidence. This is not an admissibility gate replacement.                                   | `traceUserPrefixValue`, `evaluatePaymentSource`, `evaluateSettlementFunding`       |
